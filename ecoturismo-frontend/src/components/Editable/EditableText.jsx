...
const EditableText = ({ 
  ...
  const handleSave = () => {
    console.log('💾 Guardando:', elementKey);
  ...
  return (
    <Component
      className={`
        ${className} 
        ${showEditBorder ? 'editable-element cursor-pointer border-2 border-dashed border-yellow-400 rounded-lg p-2 m-1 hover:bg-yellow-50 transition-all' : ''}
        relative
      `}
      onClick={handleClick}
      {...props}
      style={{
        ...props.style,
        minHeight: showEditBorder ? '40px' : 'auto'
      }}
    >
